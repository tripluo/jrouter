...
class PathTree<V> implements Serializable {
    ...
    /**
     * 添加路径与其相关联的值，并返回原有路径的值； 如果原有路径已存在值则保留并返回该值，不存在则返回 null。
    ...
    private void fillMatchParameters(TreeNode<V> matcher, String[] paths, Map<String, String> matchParameters) {
        if (matchParameters != null) {
            /*
             * //parse the * parameters String fullCode =
             * Integer.toBinaryString(matcher.code); //0 is the root '/' for (int i = 1; i
             * < fullCode.length(); i++) { //if 0 means '*' if (fullCode.charAt(i) == '0')
    ...
    private static class TreeNode<V> implements Serializable {
        ...
        /**
         * 根节点到此节点的路径代码（二进制）。根节点代码为1。 0代表匹配，1代表确切字符串；多匹配路径取代码最大的为最匹配路径。
         */
        private int code = -1;
        ...
        /**
         * 在此节点上添加子路径。 如果原有子路径节点已存在则返回原有的节点，否则添加并返回新增的子节点。
        ...
        /**
         * 添加叶子节点的路径与其相关联的值。 如果原叶子节点路径已存在值则返回其原有的值，否则返回 null。
         * @param paths 子节点全路径解析后的路径字符串数组。
         * @param child 指定的子路径名称。
        ...
        /**
         * 设置此节点下子节点路径的代码、索引/值类数组等信息。 如果子节点路径包含键匹配，则将其路径名称设为'*'用于路径的查询。
         * @param child 子节点。
         * @param paths 子节点全路径解析后的路径字符串数组。
         */
        ...
        /**
         * 设置叶子节点的索引/值数组。 并处理连续'*'路径（如果有）为*,*1,*2...
         * @param leaf 叶子节点。
        ...
        public String toString() {
            return "TreeNode{" + "path=" + path + ", code=" + Integer.toBinaryString(code) + ", value=" + value
                    + ", children=" + children + '}';
        }
