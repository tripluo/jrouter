...
public class JavassistProxyObjectFactory {
    ...
    /**
     * 设置未匹配的方法抛出的异常类型；{@code null}值则无异，返回默认（null/0/false）; 异常类型必须包含空构造方法。
     */
    @lombok.Setter
    ...
    private String createProxyPackageName(Class<?> instanceClass, Class<?> proxiedInterface) {
        return Modifier.isPublic(proxiedInterface.getModifiers()) ? PROXY_CLASS_PACKAGE_NAME
     ...
     */
    private CtClass createInterfaceProxyClass(Class<?> instanceClass, Class<?> proxiedInterface)
        ...
        CtClass proxyClass = classPool
            .makeClass(createProxyPackageName(instanceClass, proxiedInterface) + "." + proxiedInterface.getSimpleName()
                    + PROXY_CLASS_SUFFIX + Integer.toHexString(COUNTER.getAndIncrement()));
        try {
            // 继承特定接口/抽象类/类
            ...
            proxyClass.addField(CtField.make(
                    String.format("private %s %s;", instanceClass.getName(), PROXY_CLASS_TARGET_FIELD_NAME),
                    proxyClass));
    ...
    private void createMatchedProxyMethod(CtClass clazz, Method proxiedMethod, Method target) {
        ...
        if (parameterLength != 0) {
            ...
            body.append(proxyParameterTypes[parameterLength - 1].getCanonicalName())
                .append(" p")
                .append(parameterLength - 1);
        }
